...
public class Client implements Runnable {
	...
	private IJustHaveALook forLookingStore;
	private IFastLane fastStore;
	private ICartLane cartStore;

	public Client(IJustHaveALook forLookingStore, IFastLane fastStore, ICartLane cartStore) {
		this.forLookingStore = forLookingStore;
	...
	private void _scenario1(String item, int qty, String address, String account)
		...
		Order order = fastStore.oneShotOrder(this, item, qty, address, account);
	...
	private void _scenario2(String[] items, int[] qties, String address, String account)
		...
		for (int i = 0; i < items.length; i++) {
			...
			cart = cartStore.addItemToCart(cart, this, items[i], qties[i]);
		}
		Order order = cartStore.pay(cart, address, account);
		System.out.println(order);
	}
}
